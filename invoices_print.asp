...
SQL = "Select company_id, name, contact_name, title, division, address, city, state, zip, phone, fax, email, ap_email from subscriber  Where company_id=" & company_id
SQL2 = "select id, fname, lname, total From candidate "
SQL2 = SQL2 & "left join SelfServeCandidate on candidate.company_id = SelfServeCandidate.companyID and candidate.id = selfServeCandidate.candidateID "
SQL2 = SQL2 & "where company_id=" & company_id & " and date1 like '" & sDate & "/%/" & thisYear & "%' and hide <> 'Y' and ISNULL(SelfServeCandidate.hasEntered,1) = 1"  '& " and hide <> 'Y' " 'finished
...
<link href="css/front_print.css" rel="stylesheet" type="text/css" />
</head>
                ...
                <tr align="left">
                    <td>
                        <TABLE width="100%">
                            ...
                            <tr>
                                <td><%=rs("division")%></td>
                                <td align="right">Email:&nbsp;<%=rs("email")%></td>
                            </tr>
                            <tr>
                                <td><%=rs("address")%></td>
                                <td align="right">AP Email:&nbsp;<%=rs("ap_email")%></td>
                            </tr>
                            <tr>
