...
dim rsPrice, sql, conn, blnNDiscount
blnNDiscount = false
...
rsPrice.close
set rsPrice = nothing
Set rsPrice = getSQLServerRecordset(conn, "select * from subscriber_pricing where subscriber_id = " & company_id)
...
If request.form("submit") = "Submit" then
	...
	elseif request.form("packageA") = "t" then
		...
		resultSql(5) = "Insert Into result (id,type1,detail,state,status) values (" & candidate_id & ",'Social Security Number','SSN Verification','" & state & "','N')"
...
'	end if
	...
	If db_platform = "test" Then
		Mailer.To="chris@kingtech.net"
	Else
		'Mailer.To="jennifer@identi-check.com;micah@identi-check.com;matt@kingtech.net"
		Mailer.To="jennifer@identi-check.com;micah@identi-check.com;denis@kingtech.net"
	End If
	'Mailer.To="matt@kingtech.net"
	Mailer.From="on-line@identi-check.com"

	If CInt(session("admin")) = 2 Then
		Mailer.Subject="Internally Submitted - New Candidate for Identi-check"	
	Else
		Mailer.Subject="New Candidate for Identi-check"
	End If
	Mailer.Body = body
