...
SQL = SQL & "where date1 like '" & sDate & "/%/" & thisYear & "%' and ISNULL(SelfServeCandidate.hasEntered,1) = 1 "
<!--SQL = SQL & " and hide <> 'Y'"
-->
...
<!-- *************************************** MAIN CONTENT AREA *************************************** -->
                            ...
                            If not rs.EOF then 
								...
								Do While not rs.eof
                                    ...
                                    SQL3 = "select id, fname, lname, total From candidate "
                                    SQL3 = SQL3 & "left join SelfServeCandidate on candidate.company_id = SelfServeCandidate.companyID and candidate.id = selfServeCandidate.candidateID "
                                    SQL3 = SQL3 &"where company_id=" & rs("company_id") & " and date1 like '" & sDate & "/%/" & thisYear & "%' and hide <> 'Y' and ISNULL(SelfServeCandidate.hasEntered,1) = 1"  'finished = 'Y'
                                    SQLlastInvoiceNum = "select max(invoice_num) as invoice_num from invoice"
                                    SQLcompanyInvoice = "select * from invoice where company_id = " & rs("company_id") & " and month = " & sDate & " and year = " & thisYear
