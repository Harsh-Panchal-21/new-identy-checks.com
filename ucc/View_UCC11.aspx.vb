    ...
    If dtData.Rows.Count > 0 Then
      With dtData.Rows(0)
        ...
        pdfFormFields.SetField("B", "info@identi-check.com")
				pdfFormFields.SetField("C", Environment.NewLine & "Identi-Check, Inc." & Environment.NewLine & "3 North Old State Capitol Plaza" & Environment.NewLine & "Springfield, IL 62701")
				'first debtor
        ...
        pdfFormFields.SetField("1bSfx", .Item("ud1_ind_suffix") & "")
				...
				'checkboxes
				pdfFormFields.SetField("2aA", IIf(.Item("us_sr_certified") & "" = "Y", "C", "NC"))
				pdfFormFields.SetField("2aB", IIf(.Item("us_sr") & "" = "A", "ALL", IIf(.Item("us_sr") & "" = "U", "UN", "")))

				pdfFormFields.SetField("2b", "YES")
				pdfFormFields.SetField("2c", "YES")
				...
				pdfFormFields.SetField("5", "Other")
				pdfFormFields.SetField("5bOther", "micah@identi-check.com")


				strSQL = "Select ucc_results.*, us_company_id, case when ur_file IS NULL then 'N' else 'Y' end as 'FileReady' " &
				...
				If dtResults.Rows.Count > 0 Then
					...
					Dim sbExtra As New StringBuilder
					'need to count chars, 2000 char limit
					For Each drResult As DataRow In dtResults.Rows
						If drResult.Item("FileReady") & "" = "Y" Then
							Continue For
						End If
						If iCnt > 6 And iCnt < 34 Then
							'only 6 lines for top
							sbExtra.Append(Trim(drResult.Item("ur_filing_number") & "") & " - " & drResult.Item("ur_filing_date") & " - all associated filings" & "                     ")
							If iCnt Mod 2 = 0 Then
								sbExtra.Append(vbCrLf)
							End If
							'Exit For
						ElseIf iCnt > 34 Then
							sbExtra.Append("additional on attached paper")
							Exit For
						Else
							pdfFormFields.SetField("2cRN#" & iCnt.ToString, Trim(drResult.Item("ur_filing_number") & ""))
							pdfFormFields.SetField("2cDRF#" & iCnt.ToString, drResult.Item("ur_filing_date") & "")
							pdfFormFields.SetField("2cTR#" & iCnt.ToString, "all associated filings")
							'drResult.Item("ur_name_address") & ""
						End If
						iCnt += 1
					Next

					pdfFormFields.SetField("4", sbExtra.ToString() & "")
				Else
					...
					pdfFormFields.SetField("4", .Item("us_additional") & "")
				End If
				dtResults.Dispose()
				dtResults = Nothing
