...
Partial Class debtors
  ...
  Protected Sub rgD_NeedDataSource(sender As Object, e As Telerik.Web.UI.GridNeedDataSourceEventArgs) Handles rgD.NeedDataSource
		Dim sSQL As String
		Dim sParentID As String = Session("parent_company_id") & ""
		If sParentID = "" Then sParentID = "-1"
		If Not e.IsFromDetailTable Then
			...
			sSQL = "Select *,  " &
				 "case when isnull(ud_org_name,'') <> '' then ud_org_name  " &
				 " else isnull(ud_ind_first_name,'') + ' ' + isnull(ud_ind_middle_name,'') + ' ' + isnull(ud_ind_last_name,'') + ' ' + isnull(ud_ind_suffix,'') end as 'name' " &
				 "From ucc_debtor " &
				 "Where "
			If Session("admin") & "" = "2" Then
				sSQL += "1=1"
			Else
				sSQL += "ud_company_id in (Select company_id from subscriber where ucc_app = 'Y' and (company_id = " & Session("company_rid") & " or parent_company_id = " & sParentID & " or parent_company_id = " & Session("company_rid") & ")) "
			End If
			'Session("parent_company_id")
			'"Where ud_company_id in (Select company_id from subscriber where ucc_app = 'Y' and (company_id = " & Session("company_rid") & " or parent_company_id = " & Session("parent_company_id") & ")) "
			'lblMsg.Text = sSQL
    ...
    Select Case e.DetailTableView.Name
      Case "FilingSearches"
        Dim ID As String = dataItem.GetDataKeyValue("ud_id").ToString()
				Dim sSQL As String = "Select 'UCC1' as 'Doc', uf_id as id,  " &
					 "case when isnull(ud1.ud_org_name,'') <> '' then ud1.ud_org_name  " &
					 " else isnull(ud1.ud_ind_first_name,'') + ' ' + isnull(ud1.ud_ind_middle_name,'') + ' ' + isnull(ud1.ud_ind_last_name,'') + ' ' + isnull(ud1.ud_ind_suffix,'') end as 'name', " &
					 "case when ISNULL(ufad_id,'') <> '' then ufad_id else '' end as 'ad_id', " &
					 "case when ISNULL(ufap_id,'') <> '' then ufap_id else '' end as 'ap_id',uf_date_submitted as 'date_submitted' " &
					 "From ucc_financing inner join ucc_debtor as ud1 on uf_debtor1 = ud1.ud_id and ud1.ud_id = " & ID &
					 "left outer join ucc_financing_ad on ufad_uf_id = uf_id " &
					 "left outer join ucc_financing_ap on ufap_uf_id = uf_id " &
					 "union all " &
					 "Select 'UCC3' as 'Doc', u3_id as id, " &
					 "case when isnull(ud1.ud_org_name,'') <> '' then ud1.ud_org_name " &
					 " else isnull(ud1.ud_ind_first_name,'') + ' ' + isnull(ud1.ud_ind_middle_name,'') + ' ' + isnull(ud1.ud_ind_last_name,'') + ' ' + isnull(ud1.ud_ind_suffix,'') end as 'name', " &
					 "'' as 'ad_id', '' as 'ap_id', u3_date_submitted as 'date_submitted' " &
					 "From ucc3 inner join ucc_debtor as ud1 on u3_new_debtor = ud1.ud_id and ud1.ud_id = " & ID &
					 "" &
					 "" &
					 "union all " &
					 "Select 'UCC11' as 'Doc', us_id as id, " &
					 "case when isnull(ud1.ud_org_name,'') <> '' then ud1.ud_org_name  " &
					 " else isnull(ud1.ud_ind_first_name,'') + ' ' + isnull(ud1.ud_ind_middle_name,'') + ' ' + isnull(ud1.ud_ind_last_name,'') + ' ' + isnull(ud1.ud_ind_suffix,'') end as 'name', " &
					 "'' as 'ad_id', '' as 'ap_id',us_date_submitted as 'date_submitted' " &
