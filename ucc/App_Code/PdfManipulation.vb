...
Public Class PdfManipulation
    ...
    Try
      ...
      For i As Integer = fromPageNum To toPageNum Step 1
        ...
        If Not IsNothing(pageBytes) Then
          token = New iTextSharp.text.pdf.PRTokeniser(New iTextSharp.text.pdf.RandomAccessFileOrArray(pageBytes))
    ...
    Try
      ...
      For i As Integer = fromPageNum To toPageNum Step 1
        ...
        If Not IsNothing(pageBytes1) AndAlso Not IsNothing(pageBytes2) Then
          token1 = New iTextSharp.text.pdf.PRTokeniser(New iTextSharp.text.pdf.RandomAccessFileOrArray(pageBytes1))
          token2 = New iTextSharp.text.pdf.PRTokeniser(New iTextSharp.text.pdf.RandomAccessFileOrArray(pageBytes2))
  ...
  Public Shared Sub ExtractPdfPage(ByVal sourcePdf As String, ByVal pageNumberToExtract As Integer, ByVal outPdf As String)
    ...
    Try
      ...
      'Dim pdfStamper = New PdfStamper(reader, memStream)
      'Dim pdfFormFields As AcroFields = pdfStamper.AcroFields

      doc = New iTextSharp.text.Document(reader.GetPageSizeWithRotation(1))
      pdfCpy = New iTextSharp.text.pdf.PdfCopy(doc, New FileStream(outPdf, FileMode.Create))
      doc.Open()
      page = pdfCpy.GetImportedPage(reader, pageNumberToExtract)
      pdfCpy.AddPage(page)
      pdfCpy.SetMergeFields()
      pdfCpy.AddDocument(reader)
      'pdfCpy.CopyAcroForm(reader)
      'pdfCpy.AcroForm = reader.AcroForm
      doc.Close()
      reader.Close()
    Catch ex As Exception
      Throw ex
