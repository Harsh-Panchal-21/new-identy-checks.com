...
Public Class SOS_XML
	...
	Public Function Submit_Filing(ByVal sUF_ID As String, ByRef sDocReceiptID As String, ByRef sPacketNum As String) As String
		...
		Dim sPN As String = "1" & Right("000000000" & UCCXML_Acct_Num, 9) & Now.Year & Right("00" & sMon, 2) & Right("00" & sDay, 2) &
		...
		Try
			RunCommand("insert into ucc_xml_out_result (uxor_date,uxor_xml,uxor_type,uxor_type_id) values (getdate(),'" & responseFromServer.Replace("'", "''") & "','UCC1'," & sUF_ID & ")")
		Catch ex As Exception
			'do nothing
		End Try
		sRtn = ""
		If responseFromServer.IndexOf("DocumentReceiptID") > 0 Then
			...
			Dim packet_num As XmlElement = retDoc.SelectSingleNode("//Document/Record/PacketNum")
			...
			Else
				Try
					...
					sPacketNum = packet_num.InnerText
				Catch ex As Exception
	...
	Public Function Submit_FilingWI(ByVal sUF_ID As String, ByRef sDocReceiptID As String, ByRef sPacketNum As String) As String
		System.Net.ServicePointManager.SecurityProtocol = System.Net.SecurityProtocolType.Tls12
		Dim uccClient As New WDFI.UCCFilingClient()
		uccClient.Open()
		...
		Dim responseFromServer As String = uccClient.Filing(UCCXML_UserID_WI, UCCXML_Password_WI, strXML)
		Try
			RunCommand("insert into ucc_xml_out_result (uxor_date,uxor_xml,uxor_type,uxor_type_id) values (getdate(),'" & responseFromServer.Replace("'", "''") & "','UCC1'," & sUF_ID & ")")
		Catch ex As Exception
			'do nothing
	...
	Public Function Submit_Amendment(ByVal sU3_ID As String, ByRef sDocReceiptID As String, ByRef sPacketNum As String) As String
		...
		Dim responseFromServer As String = reader.ReadToEnd()
		Try
			RunCommand("insert into ucc_xml_out_result (uxor_date,uxor_xml,uxor_type,uxor_type_id) values (getdate(),'" & responseFromServer.Replace("'", "''") & "','UCC3'," & sU3_ID & ")")
		Catch ex As Exception
			'do nothing
		End Try
	...
	Public Function Submit_AmendmentWI(ByVal sU3_ID As String, ByRef sDocReceiptID As String, ByRef sPacketNum As String) As String
		System.Net.ServicePointManager.SecurityProtocol = System.Net.SecurityProtocolType.Tls12
		Dim uccClient As New WDFI.UCCFilingClient()
		uccClient.Open()
		...
		If responseFromServer.IndexOf("DocumentReceiptID") > 0 Then
			...
			Else
				Try
					...
					sPacketNum = packet_num.InnerText
				Catch ex As Exception
					_log.Error("Error assigning docreciptid and packetnum - " & ex.ToString)
				End Try
			End If
			'status.Attributes("value") 'should be "OK"
			'sRtn = responseFromServer
	...
	Public Function StatusRequestWI(ByVal sDocRefNum As String) As String
		System.Net.ServicePointManager.SecurityProtocol = System.Net.SecurityProtocolType.Tls12
		Dim uccClient As New WDFI.UCCFilingClient()
		uccClient.Open()

		Dim responseFromServer As String = uccClient.StatusRequest(UCCXML_UserID_WI, UCCXML_Password_WI, sDocRefNum)
		Dim sRtn As String = ""
		sRtn = ""
		sRtn = sRtn & "<br>" & vbCrLf & responseFromServer

		'WI returns filingNo, filingStatus and filingFee
		'but should cause async update to hit our server like IL
	...
	Public Function RetrieveDocumentWI(ByVal sFileNum As String, ByRef byteTemp As Byte()) As String
		System.Net.ServicePointManager.SecurityProtocol = System.Net.SecurityProtocolType.Tls12
		Dim uccClient As New WDFI.UCCFilingClient()
		uccClient.Open()
		Dim sRtn As String = ""
		'uccClient.RetrieveImage(UCCXML_UserID_WI, UCCXML_Password_WI,sFileNum) as object
		'uccClient.RetrieveDocument(UCCXML_UserID_WI, UCCXML_Password_WI,sFileNum) As Object
		'byteTemp = uccClient.RetrieveDocument(UCCXML_UserID_WI, UCCXML_Password_WI, sFileNum)
		Dim obj = uccClient.RetrieveImage(UCCXML_UserID_WI, UCCXML_Password_WI, sFileNum)
		'_log.Info("File: " & obj.ToString)
		byteTemp = obj
		sRtn = "OK"
		'sRtn = DirectCast(response, HttpWebResponse).StatusDescription
		'Dim buffer As Byte() = New Byte(4095) {}
	...
	Public Sub BuildFiling(ByVal sUF_ID As String, Optional ByVal sState As String = "IL")
		...
		If dtFile.Rows.Count > 0 Then
			With dtFile.Rows(0)
				...
				If .Item("uf_reference_data") & "" <> "" Or sState = "IL" Then
	...
	Public Sub Build_Amendment(ByVal sU3_ID As String, Optional ByVal sState As String = "IL")
		Dim sSQL As String = "Select ucc3.*, u3ad.*, u3ap.*, " &
			"ud1.ud_id as ud1_id,ud1.ud_org_name as ud1_org_name,ud1.ud_ind_last_name as ud1_ind_last_name,ud1.ud_ind_first_name as ud1_ind_first_name,ud1.ud_ind_middle_name as ud1_ind_middle_name,ud1.ud_ind_suffix as ud1_ind_suffix,ud1.ud_mailing_address as ud1_mailing_address,ud1.ud_city as ud1_city,ud1.ud_state as ud1_state,ud1.ud_zipcode as ud1_zipcode,ud1.ud_country as ud1_country,ud1.ud_d1 as ud1_d1,ud1.ud_org_type as ud1_org_type,ud1.ud_org_jurisdiction as ud1_org_jurisdiction,ud1.ud_org_id as ud1_org_id,ud1.ud_org_id_none as ud1_org_id_none, " &
			"ud2.ud_id as ud2_id,ud2.ud_org_name as ud2_org_name,ud2.ud_ind_last_name as ud2_ind_last_name,ud2.ud_ind_first_name as ud2_ind_first_name,ud2.ud_ind_middle_name as ud2_ind_middle_name,ud2.ud_ind_suffix as ud2_ind_suffix,ud2.ud_mailing_address as ud2_mailing_address,ud2.ud_city as ud2_city,ud2.ud_state as ud2_state,ud2.ud_zipcode as ud2_zipcode,ud2.ud_country as ud2_country,ud2.ud_d1 as ud2_d1,ud2.ud_org_type as ud2_org_type,ud2.ud_org_jurisdiction as ud2_org_jurisdiction,ud2.ud_org_id as ud2_org_id,ud2.ud_org_id_none as ud2_org_id_none, " &
			"ud3.ud_id as ud3_id,ud3.ud_org_name as ud3_org_name,ud3.ud_ind_last_name as ud3_ind_last_name,ud3.ud_ind_first_name as ud3_ind_first_name,ud3.ud_ind_middle_name as ud3_ind_middle_name,ud3.ud_ind_suffix as ud3_ind_suffix,ud3.ud_mailing_address as ud3_mailing_address,ud3.ud_city as ud3_city,ud3.ud_state as ud3_state,ud3.ud_zipcode as ud3_zipcode,ud3.ud_country as ud3_country,ud3.ud_d1 as ud3_d1,ud3.ud_org_type as ud3_org_type,ud3.ud_org_jurisdiction as ud3_org_jurisdiction,ud3.ud_org_id as ud3_org_id,ud3.ud_org_id_none as ud3_org_id_none, " &
			"ud4.ud_id as ud4_id,ud4.ud_org_name as ud4_org_name,ud4.ud_ind_last_name as ud4_ind_last_name,ud4.ud_ind_first_name as ud4_ind_first_name,ud4.ud_ind_middle_name as ud4_ind_middle_name,ud4.ud_ind_suffix as ud4_ind_suffix,ud4.ud_mailing_address as ud4_mailing_address,ud4.ud_city as ud4_city,ud4.ud_state as ud4_state,ud4.ud_zipcode as ud4_zipcode,ud4.ud_country as ud4_country,ud4.ud_d1 as ud4_d1,ud4.ud_org_type as ud4_org_type,ud4.ud_org_jurisdiction as ud4_org_jurisdiction,ud4.ud_org_id as ud4_org_id,ud4.ud_org_id_none as ud4_org_id_none " &
			"From ucc3 left outer join ucc_debtor as ud1 on ucc3.u3_new_debtor = ud1.ud_id " &
			"left outer join ucc3_ad as u3ad on ucc3.u3_id = u3ad_u3_id " &
			"left outer join ucc3_ap as u3ap on ucc3.u3_id = u3ap_u3_id " &
			"left outer join ucc_debtor as ud2 on u3ap.u3ap_debtor2 = ud2.ud_id " &
			"left outer join ucc_debtor as ud3 on u3ap.u3ap_debtor3 = ud3.ud_id " &
			"left outer join ucc_debtor as ud4 on u3ap.u3ap_debtor4 = ud4.ud_id " &
			"Where u3_id = " & sU3_ID
		Dim dtFile As DataTable = FillDataTable(sSQL)
		If dtFile.Rows.Count > 0 Then
			With dtFile.Rows(0)
				...
				If .Item("u3_reference_data") & "" <> "" Or sState = "IL" Then
					Dim rec_submitter_ref As XmlElement = doc.CreateElement("SubmitterRef")
	...
	Public Function Build_Debtor_Name(ByVal sOrgName As String, ByVal sIndLastName As String, ByVal sIndFirstName As String,
		ByVal sIndMiddleName As String, ByVal sIndSuffix As String, ByVal sMailAddress As String, ByVal sCity As String,
		ByVal sState As String, ByVal sPostalCode As String, ByVal sCounty As String, ByVal sCountry As String,
		ByVal sTaxID As String, ByVal sOrgType As String, ByVal sOrgJuris As String, ByVal sOrgID As String,
	...
	Public Function Build_Secured(ByVal sOrgName As String, ByVal sIndLastName As String, ByVal sIndFirstName As String,
		ByVal sIndMiddleName As String, ByVal sIndSuffix As String, ByVal sMailAddress As String, ByVal sCity As String,
		ByVal sState As String, ByVal sPostalCode As String, ByVal sCounty As String, ByVal sCountry As String,
