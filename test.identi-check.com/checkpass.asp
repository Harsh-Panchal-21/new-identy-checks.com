...
If request.form("Send") = "SUBMIT" Then
    ...
    Set rs = DB.Query(SQL,Array(UserID,Request.form("Password"),enc_pass))
      ...
      if rs("AccountLockout") = "Y" then
        ...
        Response.redirect "default.asp?msg=Login Failed, Account lockout still in effect. Lockout has been extended 10 minutes."
      else
      ...
      Set rsU = nothing
			...
			'msg1="Login failed, please try again"
			Response.redirect "default.asp?msg=" & msg1
		end if
		rs.close
		set rs = nothing
		'conn.close
