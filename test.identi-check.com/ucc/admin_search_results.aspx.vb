...
Partial Class admin_search_results
  ...
  Protected Sub btnSearch_Click(sender As Object, e As EventArgs) Handles btnSearch.Click
    ...
    Dim sSQL As String = "Select us_company_id, ur_id, ur_filing_number, ur_filing_date, ur_name_address, ur_secured_party " & _
      "From ucc_results inner join ucc_search on ur_us_id = us_id " & _
  ...
  Protected Sub dgFiling_ItemCommand(source As Object, e As DataGridCommandEventArgs) Handles dgFiling.ItemCommand
    ...
    If e.CommandName = "Select" Then
      ...
      'us_company_id, ur_id, ur_filing_number, ur_filing_date, ur_name_address, ur_secured_party From ucc_results
      Using myConnection As New SqlConnection(CNNStr)
        Dim sID As String = dgFiling.DataKeys(e.Item.ItemIndex).ToString

        Dim SQL As String = "SELECT * FROM ucc_results WHERE ur_id = @ID"
        ...
        Dim sUS As String = ""
        Dim sFiling As String = ""
        Dim sDate As String = ""
        Dim sName As String = ""
        Dim sSecured As String = ""
        Dim sCharge As String = ""

        If myReader.Read Then
          myByte = myReader("ur_file")
          sUS = myReader("ur_us_id")
          sFiling = myReader("ur_filing_number")
          sDate = myReader("ur_filing_date") & ""
          sName = myReader("ur_name_address")
          sSecured = myReader("ur_secured_party")
          sCharge = myReader("ur_charge")
        End If
        myReader.Close()

        Dim myUpCmd As SqlCommand
        SQL = "Insert Into ucc_results (ur_us_id,ur_filing_number,ur_filing_date,ur_name_address,ur_secured_party,ur_file,ur_date_entered,ur_charge) " & _
          "Values (@us_id, @file_no, @file_date, @name_address, @secured_party, @ImageData, getdate(), @charge)"
        myUpCmd = New SqlCommand(SQL, myConnection)
        myUpCmd.Parameters.AddWithValue("@us_id", Request.QueryString("us"))
        myUpCmd.Parameters.AddWithValue("@file_no", sFiling)
        myUpCmd.Parameters.AddWithValue("@file_date", If(sDate.Trim = "", DBNull.Value, sDate))
        myUpCmd.Parameters.AddWithValue("@name_address", sName)
        myUpCmd.Parameters.AddWithValue("@secured_party", sSecured)
        myUpCmd.Parameters.AddWithValue("@charge", sCharge)
        If Not myByte Is Nothing Then
          myUpCmd.Parameters.AddWithValue("@ImageData", myByte)
        Else
          myUpCmd.Parameters.Add("@ImageData", SqlDbType.VarBinary, -1)
          myUpCmd.Parameters("@ImageData").Value = DBNull.Value
        End If
        myUpCmd.ExecuteNonQuery()
