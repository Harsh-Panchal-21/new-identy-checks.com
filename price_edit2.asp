...
'	current_price = request.QueryString("price")
	...
	'if opt_cost <> current_price then
		...
		SQL = "update candidate set total = 39.60 where id = " & id
		conn.execute SQL
            ...
            <table width="590px" cellpadding="0" cellspacing="0" border="0">
                <tr align="left">
                    ...
                    <center>
                        ...
                        <table id="table_reg">
	                        <form action='price_edit2.asp?id=<%=id%>&opt=<%=rsOpt("option_name")%>&company_id=<%=company_id%>&price=<%=rsOpt("option_price")%>' method='post' id=form1 name=form1>
