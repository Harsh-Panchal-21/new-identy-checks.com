...
Else
	...
	strQS = ""
End If
SQL = SQL & " And id in (select id from result where status = 'P' and show <> 'N') "
...
<!-- *************************************** MAIN CONTENT AREA *************************************** -->
           <table width="900px" cellpadding="0" cellspacing="0" border="0">
			...
			If not rs.EOF then   
				...
				If Page > 1 Then
					Response.write "<a href='admin_pending.asp?by=" & sort_by & "&dir=" & sort_dir & "&page=" & (Page - 1) & strQS & "'>&lt;&lt; Previous Page</a>"
				End If
				Response.write "&nbsp;&nbsp;"
				If Page < intPageCount Then
					Response.write "<a href='admin_pending.asp?by=" & sort_by & "&dir=" & sort_dir & "&page=" & (Page + 1) & strQS & "'>Next Page &gt;&gt;</a>"
				End If	
				
				Response.Write "<CENTER><TABLE id='table_reg'>"
				Response.Write "<TR>"
				Response.Write "<TH>Number</th>"
				Response.WRITE "<th><a style='color: white;' href='admin_pending.asp?by=company&dir=" & iif(sort_by = "company" and sort_dir = "asc","desc","asc") & strQS & "'>Company</a></th>"
				Response.WRITE "<th><a style='color: white;' href='admin_pending.asp?by=lname,fname&dir=" & iif(sort_by = "lname,fname" and sort_dir = "asc","desc","asc") & strQS & "'>Name</a></th>"
				Response.WRITE "<th><a style='color: white;' href='admin_pending.asp?by=date1&dir=" & iif(sort_by = "date1" and sort_dir = "asc","desc","asc") & strQS & "'>Date</a></th>" 
